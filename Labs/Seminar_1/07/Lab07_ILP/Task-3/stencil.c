  ...
  for(i = STENCIL_SZ; i < N-STENCIL_SZ; i++)
    for(j = STENCIL_SZ; j < N-STENCIL_SZ; j++) {
      ...
      for(k = -STENCIL_SZ; k <= STENCIL_SZ; k++)
        sum += grid_src[i*N+j+k].x;
      */
     // new (one fused loop) — keeps center counted twice (k=0), same as v1:
     for(k = -STENCIL_SZ; k <= STENCIL_SZ; k++) {
...
void apply_stencil_v3(const GridPt* grid_src, GridPt* grid_dst, int N) {
  ...
  const int blockSz = 32; // NOTE: CHOOSE BLOCKSIZE HERE! (blockSz = N_eff is like no blocking)
  /*if(N_eff % blockSz != 0) {
  ...
  for (blockIdx = start; blockIdx < stop; blockIdx += blockSz) {
  int jStart = blockIdx;
  int jStop  = (jStart + blockSz < stop) ? (jStart + blockSz) : stop;
